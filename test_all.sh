...
echo -e "${CYAN}${BOLD}[1/3] Environment Check...${NC}"
...
echo ""
echo -e "${CYAN}${BOLD}[2/3] Running All Tests...${NC}"
...
if npx vitest run --reporter=verbose; then
    echo ""
    echo -e "${CYAN}${BOLD}[3/3] Final Verification...${NC}"
    ...
    echo ""
    echo -e "${CYAN}${BOLD}[3/3] Final Verification...${NC}"
    echo -e "\033[0;31m${BOLD}✗ SOME TESTS FAILED!${NC}"
    echo ""
    exit 1
fi
