...
import { generateMusicData } from '../../../src/engine/generator.js';
...
describe('Key Signature Selection UI', () => {
  ...
  it('should only return selected keys in config', () => {
    ...
    const keyBb = document.getElementById('key-Bb');
  ...
  it('reproduction: C and Bb selected with Chromatic should only result in C or Bb signatures', () => {
    ...
    // Uncheck all first
    document.querySelectorAll('#key-signatures input').forEach(cb => cb.checked = false);
